...
async function loadInitialData() {
  try {
    ...
    const [salonDoc, servicesSnapshot] = await Promise.all([
      getDoc(salonInfoRef),
      getDocs(servicesRef),
    ]);
...
function renderServices() {
  ...
  if (services.length === 0) {
    servicesListEl.innerHTML =
      '<p class="text-center text-gray-500">Ingen tjenester er lagt til enda.</p>';
    return;
  }
  services
...
async function renderTimeSlots(dateStr) {
  ...
  if ([openHour, openMin, closeHour, closeMin].some(isNaN)) {
    slotsContainerEl.innerHTML =
      '<p class="col-span-full text-center text-red-500">Ugyldig tidsformat i databasen.</p>';
    return;
  }
  ...
  try {
    ...
    slotsContainerEl.innerHTML =
      '<p class="col-span-full text-center text-red-500">Kunne ikke hente bestillinger.</p>';
    return;
  }
